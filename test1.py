...
path = os.path.join(path, '01 Data')
filename = os.path.join(path, 'data.csv')
if not os.path.exists(filename):
    with open(filename, 'w') as f:
        f.write('date,name,party,facebook,twitter,location\n')
...
for candidate in candidates:
    ...
    #print('debug')
    twitter_followers = t.users.show(screen_name=tw)['followers_count']
    twitter_location = t.users.show(screen_name=tw)['location']
    #print('debug')
    fb_likes = requests.get(basefb + fb + params).json()['likes']
    writer.writerow((date, name, party, fb_likes, twitter_followers, twitter_location))
f.close()
print('debug')
